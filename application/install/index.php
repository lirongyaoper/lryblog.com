...
define('APPDIR', _dir_path(substr(dirname(__FILE__), 0, -8)));//"/home/lirongyao0916/Projects/lryblog.com/application/" 
define('SITEDIR', dirname(APPDIR).DIRECTORY_SEPARATOR);
define("VERSION", 'RYCMS V1.0');

if(is_file(SITEDIR.'cache'.DIRECTORY_SEPARATOR.'install.lock')){
    exit("RYCMS程序已运行安装，如果你确定要重新安装，请先从FTP中删除 cache/install.lock！");
}
...
switch ($step) {
    ...
    case '2':
        ...
        if (strstr(gethostbyname($_SERVER['HTTP_HOST']), $_SERVER['SERVER_ADDR']) && !in_array(check_url($domain.'admin'), array(200, 550, 302, 0))){
            ...
            $rewrite_module = '<span class="correct_span error_span">&radic;</span> 未开启，<a href="javascript:;" onclick="javascript:location.reload();">刷新</a>或<a href="https://www.rycms.com/dongtai/121.html" target="_blank">查看教程</a>';
            $err++;
        } else {
