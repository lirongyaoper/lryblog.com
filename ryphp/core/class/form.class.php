...
class form {
	...
	public static function input($name = '', $value = '', $required=false, $width = 0, $attribute='') {
		$string = '<input class="lry-input-text" ';
	...
	public static function textarea($name = '', $value = '', $required=false, $width = 0, $attribute='') {
		$string = '<textarea class="textarea lry-textarea" name="'.$name.'" id="'.$name.'" ';
	...
	public static function select($name, $val = 0, $array = array(), $default_option = '', $attribute='') {
		$string = '<select name="'.$name.'" id="'.$name.'" class="select lry-select" '.$attribute.'>';
	...
	public static function checkbox($name, $val = '', $array = array()) {
		...
		foreach($array as $value) {
			...
			$string .= '<input type="checkbox" class="lry-checkbox" name="'.$name.'[]" id="'.$name.'_'.$i.'" '.$checked.' value="'.$key.'">'.$value;
			$string .= '</label>';
			$i++;
		}
		return $string;
	...
	public static function radio($name, $val = 0, $array = array()) {
		...
		foreach($array as $value) {
			...
			$string .= '<input type="radio" class="lry-radio" name="'.$name.'" id="'.$name.'_'.$key.'" '.$checked.' value="'.$key.'">'.$value;
			$string .= '</label>';
		}
		return $string;
	}
	...
	public static function image($name, $val = '', $style = '', $iscropper = false, $attribute = '') {	
		...
		$string = '<input type="hidden" name="'.$name.'_attid" id="'.$name.'_attid" value="0"><input class="input-text uploadfile" type="text" name="'.$name.'"  value="'.$val.'"  onmouseover="lry_img_preview(\''.$name.'\', this.value)" onmouseout="layer.closeAll();" id="'.$name.'" style="'.$style.'" '.$attribute.'> <a href="javascript:;" onclick="lry_upload_att(\''.U('attachment/api/upload_box', array('module'=>ROUTE_M, 'pid'=>$name), false).'\')" class="btn btn-primary radius upload-btn"><i class="lry-iconfont lry-iconshangchuan"></i> 浏览文件</a>';
	...
	public static function images($name, $val = '', $n = 20) {
		...
		if($val){
			...
			foreach($arr as $key => $val){
				$string .= '<li>文件：<input type="text" name="'.$name.'[url][]" value="'.$val['url'].'" id="'.$name.'_'.$key.'" onmouseover="lry_img_preview(\''.$name.'_'.$key.'\', this.value)" onmouseout="layer.closeAll();" class="input-text lry-input-url"> 描述：<input type="text" name="'.$name.'[alt][]" value="'.$val['alt'].'" class="input-text lry-input-alt"><a href="javascript:;" class="secondary" onclick="lry_move_li(this, 1);">上移</a> <a href="javascript:;" class="secondary" onclick="lry_move_li(this, 0);">下移</a> <a href="javascript:;" class="danger" onclick="lry_delete_li(this);">删除</a></li>';
			}
		}					
		$string .= 	'</ul></fieldset>
				<a href="javascript:;" onclick="lry_upload_att(\''.U('attachment/api/upload_box', array('module'=>ROUTE_M, 'pid'=>$name, 'n'=>$n), false).'\')" class="btn btn-primary radius upload-btn mt-5"><i class="lry-iconfont lry-iconshangchuan"></i> 浏览文件</a> <a href="javascript:;" onclick="lry_add_attachment(\''.$name.'\')" class="btn btn-secondary radius upload-btn mt-5"><i class="lry-iconfont lry-icontianjia"></i> 添加远程地址</a>';
	...
	public static function cropper($cid, $spec=2) {		
		$string = '<a href="javascript:;" onclick="lry_img_cropper(\''.$cid.'\', \''.U('attachment/api/img_cropper', array('spec'=>$spec), false).'\')" class="btn btn-secondary radius upload-btn"><i class="lry-iconfont lry-iconcaijian"></i> 裁剪图片</a>';
	...
	public static function attachment($name, $val = '', $style='', $attribute='') {
		$style = $style ? $style : 'width:370px';		
		$string = '<input type="hidden" name="'.$name.'_attid" id="'.$name.'_attid" value="0"><input class="input-text uploadfile" type="text" name="'.$name.'"  value="'.$val.'"  id="'.$name.'" style="'.$style.'" '.$attribute.'> <a href="javascript:;" onclick="lry_upload_att(\''.U('attachment/api/upload_box', array('module'=>ROUTE_M, 'pid'=>$name, 't'=>2), false).'\')" class="btn btn-primary radius upload-btn"><i class="lry-iconfont lry-iconshangchuan"></i> 浏览文件</a>';
	...
	public static function attachments($name, $val = '', $n = 20) {
		...
		if($val){
			$arr = string2array($val);
			foreach($arr as $key => $val){
				$string .= '<li>文件：<input type="text" name="'.$name.'[url][]" value="'.$val['url'].'" id="'.$name.'_'.$key.'" class="input-text lry-input-url"> 描述：<input type="text" name="'.$name.'[alt][]" value="'.$val['alt'].'" class="input-text lry-input-alt"><a href="javascript:;" class="secondary" onclick="lry_move_li(this, 1);">上移</a> <a href="javascript:;" class="secondary" onclick="lry_move_li(this, 0);">下移</a> <a href="javascript:;" class="danger" onclick="lry_delete_li(this);">删除</a></li>';
			}
		}					
		$string .= 	'</ul></fieldset>
				<a href="javascript:;" onclick="lry_upload_att(\''.U('attachment/api/upload_box', array('module'=>ROUTE_M, 'pid'=>$name, 'n'=>$n, 't'=>2), false).'\')" class="btn btn-primary radius upload-btn mt-5"><i class="lry-iconfont lry-iconshangchuan"></i> 浏览文件</a> <a href="javascript:;" onclick="lry_add_attachment(\''.$name.'\')" class="btn btn-secondary radius upload-btn mt-5"><i class="lry-iconfont lry-icontianjia"></i> 添加远程地址</a>';
