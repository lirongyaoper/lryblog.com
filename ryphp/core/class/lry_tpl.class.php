<?php
/**
 * lry_tpl.class.php  lry_tpl模板解析类
 *
 * @author           李荣耀  
...
class lry_tpl {
	...
	public function tpl_replace($str) {
		...
		$str = preg_replace_callback("/".$this->template_tag_left."m:(\w+)\s+([^}]+)".$this->template_tag_right."/i", array($this, 'lry_tag_callback'), $str);

		$str = "<?php defined('IN_LRYPHP') or exit('No permission resources.'); ?>" . $str;
		return $str;
	}	

	
	public static function lry_tag_callback($matches) {
		return self::lry_tag($matches[1],$matches[2], $matches[0]);;
	}
	 ...
	 * 解析标签
	 */	
	public static function lry_tag($action, $data, $html) {
		...
		$str .= '$tag = ryphp::load_sys_class(\'lry_tag\');';
