...
class db_mysql{
	 ...
	 * 简写：$where['字段']  = array('lrycms') 等价 $where['字段']  = array('eq', 'lrycms')
	 * @param array $where  要生成的数组,参数可以为数组也可以为字符串，建议数组。
	 * return object
	 */
	 ...
	 *                      结果为： DELETE FROM `lrycms_admin` WHERE id IN (3,4,5);
	 *
	 * @return int          返回影响行数
	 */
	...
	/**
	 * 连接查询
	 * @param $join 	string SQL语句，如lrycms_admin ON lrycms_admintype.id=lrycms_admin.id
	 * @param $type 	可选参数,默认是inner
	 * @return object
	 */	
	public function join($join, $type = 'INNER'){
		$join = str_replace('lrycms_', $this->config['db_prefix'], $join);    
