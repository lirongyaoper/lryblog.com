...
class upload {
    ...
    public function __construct($options = array()){
        ...
        foreach($options as $key=>$val){
            ...
            // 检查键名是否存在于当前类的属性中
            // get_class($this) 获取当前类名
            // get_class_vars() 获取类中所有属性
            if(!in_array($key, get_class_vars(get_class($this)))){
                continue; // 如果不存在就跳过当前循环
            }

            // 使用 setoption 方法设置属性值
            $this->setoption($key, $val);
        }

        // 设置文件上传路径,格式为: 根目录/上传目录配置/年月/日
        $this->setoption('filepath', RYPHP_ROOT.C('upload_file').'/'.date('Ym/d'));
        
        // 设置最大上传大小,从配置中获取并转换为KB单位
