...
class lry_tag{
	...
	public function centent_tag($data) {
		...
		$id = isset($data['id']) ? intval($data['id']) : 0;
		$limit = isset($data['limit']) ? $data['limit'] : '20';
		return  D('tag_content')->field('id,tag')->join('lrycms_tag b ON lrycms_tag_content.tagid=b.id')->where('lrycms_tag_content.modelid='.$modelid.' AND lrycms_tag_content.aid='.$id)->limit($limit)->order('id ASC')->select();
	}	
	...
	public function comment_list($data) {
		...
		if(isset($data['page'])){
			ryphp::load_sys_class('page','',0);
			$this->total = D('comment')->where($where)->total();
			$this->page = new page($this->total, $limit);
			$limit = $this->page->limit();
		}
	...
	public function comment_newest($data) {
		$limit = isset($data['limit']) ? $data['limit'] : '20';
		return D('comment')->field('userid,username,userpic,inputtime,content,title,url,catid,modelid')->join('lrycms_comment_data b ON lrycms_comment.commentid=b.commentid')->where('lrycms_comment.siteid='.get_siteid().' AND `status`=1')->order('id DESC')->limit($limit)->select();
	}
	...
	private function _set_model($modelid) {
		$model_info = lry_array_column(get_site_modelinfo(), 'tablename', 'modelid');
