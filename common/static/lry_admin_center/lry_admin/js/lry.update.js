	...
	$.ajax({
        ...
        success: function (msg) {
        	if(msg.status == 0){
        		...
        		lry_open('检测更新', lry_admin_url+'lry_admin_center/store/public_check_update', 500, 500);
        	}else{
        		$("#new_version").html("(发现新版本)");
        	}
        },
...
function lry_update_ignore(){
	layer.confirm('确定要忽略本次升级吗？<br><span class="c-red">不升级可能会有安全风险！</span>',function(index){
		$.ajax({
	        type: "GET",
	        url: lry_admin_url+'lry_admin_center/store/public_update_ignore',
		    dataType: "json", 
	        success: function (msg) {
	        	if(msg.status){
...
// www.lryper.com
function lry_update_exec(){
	var mymsg = layer.msg('正在升级中，请勿刷新或关闭页面！', {icon:16, time:0, shade:0.3});
	$.ajax({
        type: "GET",
        url: lry_admin_url+'lry_admin_center/store/public_system_update',
	    dataType: "json", 
        success: function (msg) {
        	if(msg.status == 0){
