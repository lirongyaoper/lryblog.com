...
function album_cancel(obj){
	if($(obj).hasClass('on')){
		...
		var file_upload_limit = lry_uploader_config.fileNumLimit;
...
jQuery(function() {
    ...
    uploader = WebUploader.create(lry_uploader_config);

    // 当有文件添加进来的时候
    uploader.on( 'fileQueued', function( file ) {
        var $li = $(
                '<div id="' + file.id + '" class="lry-file-item">' +
                    '<span class="lry-file-name" title="' + file.name + '">' + file.name + '</span><span class="lry-file-size">（' + sizecount(file.size) + '）</span><span class="lry-per"></span>' +
                '</div>'
                );
        $list.append( $li );
    });
    ...
    uploader.on( 'uploadProgress', function( file, percentage ) {
        var $li = $( '#'+file.id ),
            $percent = $li.find('.lry-progress span');

        if ( !$percent.length ) {
            $percent = $('<p class="lry-progress"><span></span></p>')
                    .appendTo( $li )
                    .find('span');
        }

        $li.find(".lry-per").text( ' - ' + (percentage * 100).toFixed(2) + '%' );
    ...
    uploader.on( 'uploadSuccess', function( file, data ) {
        ...
        $( '#'+file.id ).hide(100);
        // $( '#'+file.id ).addClass('lry-upload-done');
    });

    // 文件上传失败
    uploader.on( 'uploadError', function( file, code ) {
        var $li = $( '#'+file.id ),
            $error = $li.find('div.lry-upload-error');

        if ( !$error.length ) {
            $( '#'+file.id ).find('.lry-progress').remove();
            $error = $('<div class="lry-upload-error"></div>').appendTo( $li );
        }
    ...
    uploader.on("error", function (type) {
        if (type == "Q_EXCEED_NUM_LIMIT") {
            layer.msg("本次最多允许上传" + lry_uploader_config.fileNumLimit + "个文件！", {icon:2});
        } else if (type == "F_EXCEED_SIZE") {
            layer.msg("单文件大小不能超过" + sizecount(lry_uploader_config.fileSingleSizeLimit) + "！", {icon:2});
