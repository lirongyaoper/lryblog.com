...
function get_theme_list($m = 'index'){
	$theme_list = array();
	$list = glob(RYPHP_APP.$m.DIRECTORY_SEPARATOR.'view'.DIRECTORY_SEPARATOR.'*', GLOB_ONLYDIR);
...
function get_copyfrom($modelid = 1){
	...
	$res = lry_array_column($res, 'copyfrom');
...
function content_list_tag($catid = 0, $id = 0, $limit = 5){
	...
	$id = intval($id);
	return D('tag_content')->alias('a')->field('id,tag')->join('lrycms_tag AS b ON a.tagid=b.id')->where("modelid=$modelid AND aid=$id")->order('id ASC')->limit($limit)->select();
}
...
function delete_attachment($modelid, $id){
	...
	ryphp::load_model($upload_type, 'attachment', 0);
...
function get_site($siteid = 0, $parameter = ''){
	...
	if(!is_file(RYPHP_APP.'site/common/function/function.php')) {
		if($siteid){
	...
	include_once RYPHP_APP.'site/common/function/function.php';
 ...
 * get_category($k, 'arrchildid', true)
 */
function get_category($catid = 0, $parameter = '', $all = false){
	...
	if($catid){
		$catid_arr = lry_array_column($categoryinfo, null, 'catid');
...
function get_memberavatar($user, $type=1, $default=true) {
	...
	if($type == 1){
		...
		$data = $member_detail->field('userpic')->join('lrycms_member b ON lrycms_member_detail.userid=b.userid')->where(array('username' => $user))->find();
	}	
...
function get_field_val($value, $field, $modelid){
	...
	$data = lry_array_column($modelinfo, null, 'field');
